...
public class myGallery extends Activity implements AdapterView.OnItemSelectedListener, ViewSwitcher.ViewFactory, OnTouchListener {
            ...
            case (0):
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if (c.moveToFirst()) {
                        ...
                        tv.setText(name);
                        try {
                            Cursor c1 = sampleDB.rawQuery("SELECT name FROM " + SAMPLE_DB_NAME + " where imgPath = '" + imags.get(pos).getAbsolutePath() + "'", null);
                            if (c1.getCount() == 0) {
    ...
    public void onItemSelected(AdapterView<?> parent, View v, int position,
        ...
        try {
            ...
            sms.setVisibility(8);
            try {
                Cursor c = sampleDB.rawQuery("SELECT name FROM " + SAMPLE_DB_NAME + " where imgPath = '" + imags.get(position).getAbsolutePath() + "'", null);
                if (c.getCount() != 0) {
                    c.moveToFirst();
